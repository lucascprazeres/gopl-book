...
var pallete = []color.Color{color.Black, color.RGBA{0x32, 0xa8, 0x52, 0xff}, color.RGBA{0x8d, 0x0f, 0xd6, 0xff}}
...
func lissajous(out io.Writer) {
	...
	for i := 0; i < nframes; i++ {
		...
		for t := 0.0; t < cycles*2*math.Pi; t += res {
			...
			img.SetColorIndex(size+int(x*size+0.5), size+int(y*size+0.5), uint8(randomInt(1, 2)))
		}
		phase += 0.1
